...
Description: 网易大神-光遇每日任务爬虫 主解析文件
"""
import requests
...
---''' % (title, time.strftime("%Y-%m-%d %H:%M:%S"), title)
        ...
        # 在 markdowns 目录中写文件
        with open(os.path.join("markdowns", "%s.md" % (file_name)), "w", encoding="utf8") as m:
            m.write(md)
        ...
        return os.path.join("markdowns", "%s.md" % (file_name)), html

    def parseArticle(self, htmlData):
        '''
        解析文章内容,传入匹配出来的html,转为 markdowns 格式
        并保存处理后的HTML MD文件
        '''
